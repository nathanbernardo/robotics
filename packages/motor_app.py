...
entries = []
for i in range(6):
    tk.Label(root, text=f"Motor {i+1}:", font=("Arial", 12), bg="#f0f0f0").grid(
    ...
    tk.Button(
        ...
        bg="#4CAF50",
        fg="white",
        command=lambda i=i, e=entry: send_command(i, e),
    ).grid(row=i + 1, column=2, padx=10, pady=5, sticky="ew")

tk.Button(
    root, text="Quit", font=("Arial", 12), bg="#D32F2F", fg="white", command=root.quit
